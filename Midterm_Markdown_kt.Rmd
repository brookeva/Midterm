...
reg1 <- lm(price ~ ., data = nc_sub_200k %>% 
                                 dplyr::select(price, heatedarea, actype, 
                                               storyheigh, numfirepla, fullbaths,
                                               halfbaths, bedrooms, shape_Area, 
                                               zipcode, yearbuilt, extwall))
...
inTrain <- createDataPartition(
              ...
              p = .60, list = FALSE)
NCData.training <- NCData.sf[inTrain,] 
NCData.test <- NCData.sf[-inTrain,]  
 
reg.training <- 
  lm(SalePrice ~ ., data = as.data.frame(NCData.training) %>% 
...
NCData.sf$lagPrice <- lag.listw(spatialWeights, NCData.sf$SalePrice)
```
...
```{r}
Hidecoords.test <-  st_coordinates(NCData.test) 
...
```{r}
moranTest <- moran.mc(NCData.test$SalePrice.Error, 
...
```{r}
left_join(
  st_drop_geometry(NCData.test) %>%
    group_by(Name) %>%
    summarize(meanPrice = mean(SalePrice, na.rm = T)),
  mutate(NCData.test, predict.fe = 
                        predict(lm(SalePrice ~ Name, data = NCData.test), 
                        NCData.test)) %>%
    st_drop_geometry %>%
    group_by(Name) %>%
...
```{r}
reg.nhood <- lm(SalePrice ~ ., data = as.data.frame(NCData.training) %>% 
...
NCData.test.nhood <-
  ...
  mutate(Regression = "Neighborhood Effects",
         SalePrice.Predict = predict(reg.nhood, NCData.test),
         SalePrice.Error = SalePrice.Predict- SalePrice,
         SalePrice.AbsError = abs(SalePrice.Predict- SalePrice),
...
bothRegressions <- 
  rbind(
    dplyr::select(NCData.test, starts_with("SalePrice"), Regression, Name) %>%
      mutate(lagPriceError = lag.listw(spatialWeights.test, SalePrice.Error)),
    dplyr::select(NCData.test.nhood, starts_with("SalePrice"), Regression, Name) %>%
      mutate(lagPriceError = lag.listw(spatialWeights.test, SalePrice.Error)))  
```
